<template>
  <div>
    ...
    <div
      v-else
      class="max-w-screen bg-base-200 py-8 flex flex-col items-center"
    >
      <div class="w-full max-w-3xl mx-auto px-4">
        <h1
          class="text-2xl md:text-3xl font-bold text-center mb-8 text-base-content"
        >
          Liste des bénévoles de l'association
        </h1>
...
const loading = computed(
  () => volunteers.value === null || useAssociation.loading.value
)
...
async function initData () {
  if (!useAssociation.association.value?.associationId) {
    ...
    associationId.value =
      useAssociation.association.value.associationId || null
  }
}
...
async function handleRemoveVolunteer (volunteerId: string) {
  ...
  try {
    await useAssociation.removeVolunteerFromAssociation(
      associationId.value,
      volunteerId
    )
  } catch (error) {
