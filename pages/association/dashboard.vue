...
const totalVolunteers = computed(
  () =>
    Array.from(
      new Set(
        mockAssociations.flatMap(a => a.volunteers?.map(v => v.id) || [])
      )
    ).length
)
...
const totalParticipants = computed(() =>
  mockEvents.reduce(
    (sum, e) => sum + (e.nbParticipants || e.participants?.length || 0),
    0
  )
)
...
const retentionRate = computed(() => {
  ...
  mockEvents.forEach((event) => {
    event.volunteers?.forEach((volunteer) => {
      volunteerParticipation.set(
        volunteer.id,
        (volunteerParticipation.get(volunteer.id) || 0) + 1
      )
    })
  })
  const multiEventVolunteers = Array.from(
    volunteerParticipation.values()
  ).filter(count => count > 1).length
  return totalVolunteers.value > 0
    ? Math.round((multiEventVolunteers / totalVolunteers.value) * 100)
    : 0
})
...
const averageObjectiveCompletion = computed(() => {
  ...
  const totalCompletion = objectives.reduce(
    (sum, obj) => sum + obj.covered / obj.planned,
    0
  )
  return objectives.length > 0
    ? Math.round((totalCompletion / objectives.length) * 100)
    : 0
})
...
<template>
  ...
  >
    ...
    <!-- Main container -->
    <div
      class="container mx-auto px-3 sm:px-4 py-4 sm:py-6 lg:py-8 max-w-7xl overflow-x-hidden"
    >
      <!-- Header section -->
      <div class="mb-6 sm:mb-8">
        ...
        <p
          class="text-sm sm:text-base text-base-content opacity-70 break-words"
        >
      ...
      <!-- Date Range Picker -->
      <div
        class="bg-base-100 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8"
      >
        <div class="flex flex-col gap-4">
          <div class="flex-1">
            <h2
              class="text-base sm:text-lg font-semibold text-base-content mb-2 break-words"
            >
              Période d'analyse
            </h2>
            <p
              class="text-xs sm:text-sm text-base-content opacity-70 break-words"
            >
      ...
      >
        <div
          class="bg-base-100 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-all duration-300 border border-base-300 min-w-0"
        >
          <div class="flex items-center justify-between mb-3 sm:mb-4">
            <div
              class="w-10 h-10 sm:w-12 sm:h-12 bg-primary/10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0"
            >
              <svg
            ...
            <div class="text-right min-w-0 flex-1 ml-3">
              <div
                class="text-xl sm:text-2xl font-bold text-base-content truncate"
              >
                {{ totalAnnouncements }}
              </div>
              <div
                class="text-xs sm:text-sm text-base-content opacity-70 truncate"
              >
                Événements
              </div>
            </div>
          ...
          <div class="flex items-center gap-2">
            <div class="text-xs sm:text-sm text-success font-medium">
              +12%
            </div>
        ...
        >
          <div class="flex items-center justify-between mb-3 sm:mb-4">
            <div
              class="w-10 h-10 sm:w-12 sm:h-12 bg-success/10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0"
            >
              <svg
        ...
        >
          <div class="flex items-center justify-between mb-3 sm:mb-4">
            <div
              class="w-10 h-10 sm:w-12 sm:h-12 bg-warning/10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0"
            >
              <svg
        ...
        >
          <div class="flex items-center justify-between mb-3 sm:mb-4">
            <div
              class="w-10 h-10 sm:w-12 sm:h-12 bg-info/10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0"
            >
              <svg
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8 mb-6 sm:mb-8">
        ...
        <div
          class="bg-base-100 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 min-w-0"
        >
          <div
            class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3"
          >
            <h3
              class="text-base sm:text-lg font-semibold text-base-content break-words"
            >
              Évolution des événements
            </h3>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8 mb-6 sm:mb-8">
        ...
        >
          ...
          <div
            class="space-y-3 sm:space-y-4 max-h-96 overflow-y-auto overflow-x-hidden"
          >
            <div
      ...
      <div
        class="bg-base-100 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8 min-w-0"
      >
        <h3
          class="text-base sm:text-lg font-semibold text-base-content mb-4 sm:mb-6 break-words"
        >
        ...
        >
          <div
            class="bg-gradient-to-br from-primary/10 to-primary/5 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-primary/20 min-w-0"
          >
            <div class="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              <div
                class="w-6 h-6 sm:w-8 sm:h-8 bg-primary rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-3 h-3 sm:w-4 sm:h-4 text-primary-content"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <h4
                class="font-semibold text-base-content text-sm sm:text-base break-words"
              >
                Engagement
              </h4>
            </div>
            ...
            >
              ...
              <span class="font-medium text-primary">Publiez plus de contenu interactif</span>
              pour l'améliorer.
            </p>
          </div>

          <div
            class="bg-gradient-to-br from-success/10 to-success/5 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-success/20 min-w-0"
          >
            <div class="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              <div
                class="w-6 h-6 sm:w-8 sm:h-8 bg-success rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-3 h-3 sm:w-4 sm:h-4 text-success-content"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div
            class="bg-gradient-to-br from-warning/10 to-warning/5 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-warning/20 min-w-0"
          >
            <div class="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              <div
                class="w-6 h-6 sm:w-8 sm:h-8 bg-warning rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-3 h-3 sm:w-4 sm:h-4 text-warning-content"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <p
              class="text-xs sm:text-sm text-base-content opacity-70 break-words"
            >
              {{ averageObjectiveCompletion }}% d'atteinte moyenne des
              objectifs.
              <span class="font-medium text-warning">Ajustez vos objectifs</span>
