...
const announcements = computed<AnnouncementRow[]>(() => (announcement.getAnnouncements.value || []) as unknown as AnnouncementRow[])
...
async function searchById() {
  try {
    ...
    if (!item) noResultMsg.value = `Aucun résultat pour l'ID ${searchId.value}`
  } catch (e) {
    displayed.value = []
...
async function saveEdit() {
  if (!editable.value?._id) return
  try {
    await announcement.updateAnnouncement(editable.value._id as string, {
      nameEvent: editable.value.nameEvent,
      status: editable.value.status,
    } as any)
    const row = displayed.value.find(a => a._id === editable.value?._id)
    if (row) {
      if (editable.value.nameEvent) row.nameEvent = editable.value.nameEvent
      if (editable.value.status) row.status = editable.value.status as AnnouncementRow['status']
    }
...
<template>
  <div>
    ...
    <section class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      ...
      <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div class="flex items-center gap-2 sm:ml-auto">
          <input v-model.trim="searchId" @keyup.enter="searchById" class="input input-bordered input-sm" placeholder="Rechercher par ID" />
      ...
      <div class="overflow-x-auto bg-base-100 rounded-xl shadow">
        <table class="table w-full">
          ...
          <tbody>
            <tr v-for="a in displayed" :key="a._id">
              <td class="font-mono text-xs sm:text-sm break-all">{{ a._id }}</td>
              ...
              <td class="flex flex-wrap gap-2">
                ...
                <button class="btn btn-xs" @click="updateStatus(a._id, 'INACTIVE')">INACTIVE</button>
                <button class="btn btn-xs" @click="updateStatus(a._id, 'COMPLETED')">COMPLETED</button>
        ...
        <div v-if="!loading && displayed.length === 0" class="text-center py-8 text-base-content/70">{{ noResultMsg || 'Aucune annonce' }}</div>
      </div>
    </section>
    ...
    <div v-if="isEditOpen" class="modal modal-open">
      ...
      <div class="modal-backdrop" @click="isEditOpen = false"></div>
    </div>
  </div>
</template>
