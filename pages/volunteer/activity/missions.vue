<template>
  <div class="min-h-screen bg-gradient-to-br from-base-100 to-base-200">
    ...
    <div v-else class="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div
        class="bg-base-100 rounded-2xl shadow-lg p-6 mb-8 backdrop-blur-sm border border-base-300"
      >
          ...
          >
            ...
            <div class="relative p-6 pb-4">
              <div class="flex justify-between items-start mb-4">
                <div class="flex-1">
                  <h2
                    class="text-xl font-bold text-base-content group-hover:text-primary transition-colors duration-300 line-clamp-2"
                  >
              ...
              <div class="space-y-3">
                ...
                <div class="flex items-center gap-3">
                  <div
                    class="flex items-center justify-center w-8 h-8 bg-secondary/10 rounded-lg"
                  >
        ...
        <div v-else class="text-center py-16">
          <div class="max-w-md mx-auto">
            ...
            <p class="text-base-content opacity-70 mb-8 leading-relaxed">
              Vous n'avez pas encore participé à des missions. Découvrez les
              événements disponibles et commencez votre aventure !
            </p>
          </div>
        </div>
      </div>
...
function isVolunteerInWaitingList (
...
function isVolunteerInVolunteerList (
  announcement: Announcement
): boolean | undefined {
  if (!getUserId) {
    return false
...
async function initData () {
  try {
    ...
    if (getUserId) {
      announcements.value =
        await useVolunteer.getVolunteerAnnouncements(getUserId)
    } else {
      console.warn(
...
const filteredAnnouncements = computed(() => {
  return announcements.value
    .filter((a) => {
      if (filter.value === 'participant') {
        return isVolunteerInVolunteerList(a)
      }
      if (filter.value === 'waiting') {
        return isVolunteerInWaitingList(a)
      }
...
async function cancelVolunteer (announcement: Announcement) {
  ...
  try {
    if (isVolunteerInVolunteerList(announcement)) {
      ...
      console.warn(
        "L'utilisateur n'est pas dans la liste des participants ou en attente."
      )
      return
    }
