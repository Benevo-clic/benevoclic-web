<template>
  <div class="min-h-screen bg-gradient-to-br from-base-100 to-base-200">
    ...
    <div v-else class="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <!-- Filter and search -->
      <div
        class="bg-base-100 rounded-2xl shadow-lg p-6 mb-8 border border-base-300"
      >
        <div class="flex flex-col lg:flex-row gap-4">
          <div class="flex-1 relative">
            ...
            >
            <Search
              class="w-5 h-5 absolute left-4 top-1/2 transform -translate-y-1/2 text-base-content opacity-50"
            />
          </div>
          <div class="lg:w-48">
            <select
      ...
      <div class="flex justify-center">
        <div class="w-full md:w-4/5">
          <div v-if="displayedItems.length > 0" class="relative">
            <!-- Timeline line -->
            <div class="absolute left-6 top-0 bottom-0 w-0.5 bg-base-300" />
            <div class="space-y-8">
              <div
              ...
              >
                ...
                <div class="flex flex-col items-center z-10">
                  <div
                    class="w-16 h-16 rounded-full bg-base-100 border-4 border-primary flex flex-col items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300"
                  >
                    <span
                      class="text-xs text-primary font-bold text-center leading-tight"
                    >
                      {{ getPeriod(item) }}
                    </span>
                  </div>
                  <div
                    ...
                    class="flex-1 w-0.5 bg-base-300 mt-1"
                  />
                </div>
                <!-- Timeline card -->
                <div class="ml-6 flex-1">
                  <div
                    class="bg-base-100 rounded-2xl shadow-lg border border-base-300 p-6 transition-all duration-300 group-hover:shadow-2xl"
                  >
                    <div class="flex justify-between items-center mb-2">
                      <h3
                        class="font-semibold text-lg text-base-content group-hover:text-primary transition-colors duration-300"
                      >
                        {{ item.nameEvent }}
                      </h3>
          ...
          <div v-else class="text-center py-16">
            <div class="max-w-md mx-auto">
              <div
                class="w-24 h-24 mx-auto mb-6 bg-base-200 rounded-full flex items-center justify-center"
              >
              ...
              <p class="text-base-content opacity-70 mb-8 leading-relaxed">
                Votre historique d'activités apparaîtra ici dès que vous aurez
...
async function initData () {
  try {
    ...
    if (getUserId) {
      announcements.value =
        await useVolunteer.getPastVolunteerAnnouncement(getUserId)
    } else {
      console.warn(
...
const filteredItems = computed(() => {
  return announcements.value
    .filter((a) => {
      if (filterType.value === 'mission') {
        return isVolunteerAnnouncement(a)
      }
      if (filterType.value === 'participation') {
        return isParticipationAnnouncement(a)
      }
      return isVolunteerAnnouncement(a) || isParticipationAnnouncement(a)
    })
    .filter(a =>
      a.nameEvent.toLowerCase().includes(searchQuery.value.toLowerCase())
    )
})

function isVolunteerAnnouncement (
...
function isParticipationAnnouncement (
  announcement: Announcement
): boolean | undefined {
  if (!getUserId) {
    return false
  }
  return announcement.participants?.some(
    participation => participation.id === getUserId
  )
}
