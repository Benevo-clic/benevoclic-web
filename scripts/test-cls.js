...
try {
  ...
  console.log(
    `CLS: ${cls.toFixed(3)} ${cls <= 0.1 ? "✅" : "❌"} (objectif: ≤ 0.1)`,
  );
  console.log(
    `LCP: ${(lcp / 1000).toFixed(2)}s ${lcp <= 2500 ? "✅" : "❌"} (objectif: ≤ 2.5s)`,
  );
  console.log(
    `FCP: ${(fcp / 1000).toFixed(2)}s ${fcp <= 1800 ? "✅" : "❌"} (objectif: ≤ 1.8s)`,
  );
  ...
  const clsElements =
    report.audits["layout-shift-elements"]?.details?.items || [];
