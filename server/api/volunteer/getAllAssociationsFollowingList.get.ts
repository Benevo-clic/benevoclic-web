...
export default defineEventHandler(async event => {
  ...
  try {
    const { data } = await axios.get<AssociationVolunteerFollow[]>(
      `${apiBaseUrl}/api/v2/association/${volunteerId}/getAssociationVolunteersList`,
      {
        headers: {
    ...
    if (axios.isAxiosError(error)) {
      await ApiError.handleAxios(
        error,
        'Erreur lors de la récupération de la liste des associations suivies par le volontaire'
      )
    }
  }
})
