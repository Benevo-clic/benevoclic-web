...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.error('❌ Error in favorites-announcement/announcement/[announcementId].delete.ts:', error)
    if (axios.isAxiosError(error)) {
      ApiError.handleAxios(error, 'Erreur lors de la suppression de tous les favoris de l\'annonce')
    }
    throw createError({
      statusCode: error.statusCode || 500,
      statusMessage: error.statusMessage || 'Erreur lors de la suppression de tous les favoris de l\'annonce'
    })
  }
}) 