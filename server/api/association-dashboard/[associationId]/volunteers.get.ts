...
export default defineEventHandler(async event => {
  ...
  try {
    ...
    const url = `${apiBaseUrl}/association-dashboard/${associationId}/volunteers${queryParams.toString() ? `?${queryParams.toString()}` : ''}`
    ...
    if (axios.isAxiosError(error)) {
      ApiError.handleAxios(error, 'Erreur lors de la récupération des statistiques de bénévoles')
    }
  }
})
