...
describe('DateRangePicker', () => {
  ...
  it('should emit update:modelValue when dates change', async () => {
    ...
    expect(emissions?.[emissions.length - 1]).toEqual([{ from: '2024-01-01', to: '2024-01-31' }])
  })
  ...
  it('should emit change event when filter button is clicked', async () => {
    ...
    expect(wrapper.emitted('change')?.[0]).toEqual([{ from: '2024-01-01', to: '2024-01-31' }])
  })
  ...
  it('should have proper aria attributes for button', () => {
    ...
    const button = wrapper.find('button')
    expect(button.attributes('aria-label')).toBe('Appliquer le filtre de dates')
  })
  ...
  it('should show error message for invalid range', () => {
    ...
    expect(errorDiv.text()).toContain('La date de fin doit être postérieure à la date de début')
  })
