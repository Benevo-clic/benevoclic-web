...
describe("VolunteerEventFavoritesFilters", () => {
  ...
  describe("Rendu de base", () => {
    ...
    it("should display sort options", () => {
      ...
      expect(wrapper.vm.sortOptions[0].label).toBe(
        "Date d'événement (croissant)",
      );
      expect(wrapper.vm.sortOptions[1].label).toBe(
        "Date d'événement (décroissant)",
      );
      expect(wrapper.vm.sortOptions[2].label).toBe(
        "Date de publication (récent)",
      );
    });
  });
