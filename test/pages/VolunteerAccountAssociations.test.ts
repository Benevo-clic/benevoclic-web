...
describe('VolunteerAccountAssociations', () => {
  it('should render the main container', () => {
    ...
    expect(wrapper.find('.max-w-screen.bg-base-200.py-8.flex.flex-col.items-center').exists()).toBe(true)
  })
  ...
  it('should render the search input', () => {
    ...
    expect(searchInput.attributes('placeholder')).toBe('Rechercher une association...')
    expect(searchInput.attributes('aria-label')).toBe('Champ de saisie')
  })

  it('should render the search icon', () => {
    const wrapper = mount(MockVolunteerAccountAssociations)
    const searchIcon = wrapper.find('.absolute.inset-y-0.left-0.flex.items-center.pl-3.pointer-events-none')
