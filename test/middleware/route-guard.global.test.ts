  ...
  describe("Fonctions utilitaires", () => {
    describe("getPathWithoutLocale", () => {
      it("should remove English locale prefix", () => {
        expect(getPathWithoutLocale("/en/volunteer/account")).toBe(
          "/volunteer/account",
        );
      });

      it("should remove Spanish locale prefix", () => {
        expect(getPathWithoutLocale("/es/association/dashboard")).toBe(
          "/association/dashboard",
        );
      });

      it("should return path unchanged when no locale prefix", () => {
        expect(getPathWithoutLocale("/volunteer/account")).toBe(
          "/volunteer/account",
        );
      });
    });
    ...
    describe("isRouteAccessible", () => {
      ...
      it("should allow volunteer routes for volunteer role", () => {
        expect(
          isRouteAccessible("/volunteer/account", RoleUser.VOLUNTEER),
        ).toBe(true);
      });

      it("should deny volunteer routes for association role", () => {
        expect(
          isRouteAccessible("/volunteer/account", RoleUser.ASSOCIATION),
        ).toBe(false);
      });

      it("should allow admin access to all routes", () => {
        expect(isRouteAccessible("/volunteer/account", RoleUser.ADMIN)).toBe(
          true,
        );
        expect(
          isRouteAccessible("/association/dashboard", RoleUser.ADMIN),
        ).toBe(true);
      });
    });
    ...
    describe("getHomePageForRole", () => {
      ...
      it("should return correct home page for association", () => {
        expect(getHomePageForRole(RoleUser.ASSOCIATION)).toBe(
          "/association/dashboard",
        );
      });
      ...
      it("should add locale prefix when not French", () => {
        expect(getHomePageForRole(RoleUser.VOLUNTEER, "en")).toBe(
          "/en/volunteer",
        );
        expect(getHomePageForRole(RoleUser.ASSOCIATION, "es")).toBe(
          "/es/association/dashboard",
        );
      });
    });
  });
