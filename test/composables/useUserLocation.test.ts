...
describe('useUserLocation', () => {
  ...
  describe('Méthode loadLocationFromStorage', () => {
    ...
    it('should return cached location when valid', () => {
      const location = useUserLocation()
      const mockLocation = { latitude: 48.8566, longitude: 2.3522, timestamp: Date.now() }
    ...
    it('should return null when location expired', () => {
      ...
      expect(localStorage.removeItem).toHaveBeenCalledWith('user_location_timestamp')
    })
  })
  ...
  describe('Méthode saveLocationToStorage', () => {
    it('should save location to localStorage', () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('user_location', JSON.stringify(mockLocation))
      expect(localStorage.setItem).toHaveBeenCalledWith('user_location_timestamp', expect.any(String))
    })
  })
  ...
  describe('Méthode getCurrentLocation', () => {
    ...
    it('should handle geolocation not supported', async () => {
      ...
      await expect(location.getCurrentLocation()).rejects.toThrow('La géolocalisation n\'est pas supportée par ce navigateur')
    })
    ...
    it('should handle permission denied error', async () => {
      ...
      await expect(location.getCurrentLocation()).rejects.toThrow('Permission de géolocalisation refusée')
    })
  })
