...
export const useFavoriteAnnouncement = defineStore('favoriteAnnouncement', {
    ...
    actions: {
        ...
        async fetchFavorites(volunteerId: string, announcementId: string) {
            ...
            this.error = null;
            try {
                this.favorites = await $fetch<FavoritesAnnouncement[]>(`/api/favorites-announcement/volunteer/${volunteerId}/announcement/${announcementId}`, {
                    method: 'GET',
                    credentials: 'include',
                });
        ...
        async fetchAllFavoritesOfVolunteer(volunteerId: string) {
            ...
            this.error = null;
            try {
                this.favorites = await $fetch<FavoritesAnnouncement[]>(`/api/favorites-announcement/volunteer/${volunteerId}`, {
                    method: 'GET',
                    credentials: 'include',
                });
        ...
        async fetchFavoriteVolunteerByVolunteerId(volunteerId: string) {
            ...
            this.error = null;
            try {
                this.favoritesAnnouncementsVolunteer = await $fetch<Announcement[]>(`/api/favorites-announcement/volunteer/${volunteerId}/favorites`, {
                    method: 'GET',
                    credentials: 'include',
                });
        ...
        async fetchFavoritesByAnnouncementId(announcementId: string) {
            ...
            this.error = null;
            try {
                this.favorites = await $fetch<FavoritesAnnouncement[]>(`/api/favorites-announcement/announcement/${announcementId}`, {
                    method: 'GET',
                    credentials: 'include',
                });
        ...
        async removeByVolunteerIdAndAnnouncementId(volunteerId: string, announcementId: string) {
            ...
            this.error = null;
            try {
                await $fetch(`/api/favorites-announcement/volunteer/${volunteerId}/announcement/${announcementId}`, {
                    method: 'DELETE',
                    credentials: 'include',
                });
