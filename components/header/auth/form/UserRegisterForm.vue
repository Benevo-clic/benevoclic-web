  ...
  >
    ...
    <div class="form-control">
      ...
      <input
          ...
          aria-required="true"
          autocomplete="new-password"
          :aria-describedby="form.password !== form.confirmPassword && form.confirmPassword ? 'password-mismatch' : 'confirm-password-description'"
      />
      <div id="confirm-password-description" class="text-xs text-gray-500 mt-1">
        Confirmez votre mot de passe
      </div>
      <div id="password-mismatch" class="text-xs text-error mt-1" v-if="form.password !== form.confirmPassword && form.confirmPassword">
    ...
    <button 
      ...
      :aria-describedby="loading ? 'register-loading' : (!termsAccepted ? 'register-disabled' : undefined)"
    >
      <span v-if="loading" class="loading loading-spinner loading-sm" aria-hidden="true"></span>
      <span v-else>{{t('auth.continue')}}</span>
    </button>
    <div id="register-loading" class="sr-only" v-if="loading">
      Inscription en cours...
    </div>
    <div id="register-disabled" class="sr-only" v-if="!termsAccepted">
