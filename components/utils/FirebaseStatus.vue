    ...
    <div class="space-y-2 mb-4">
      <div class="flex items-center gap-2">
        <div
          ...
          :class="firebaseReady ? 'bg-success' : 'bg-error'"
        />
      ...
      <div class="flex items-center gap-2">
        <div
          class="w-3 h-3 rounded-full"
          :class="authReady ? 'bg-success' : 'bg-warning'"
        />
        <span>Authentification:
          {{ authReady ? "Disponible" : "Non disponible" }}</span>
      </div>

      <div class="flex items-center gap-2">
        <div
          class="w-3 h-3 rounded-full"
          :class="analyticsReady ? 'bg-success' : 'bg-warning'"
        />
  ...
  try {
    ...
    if (firebase) {
      ...
      analyticsReady.value =
        !!(firebase as any).analytics && (firebase as any).analyticsEnabled
    }
  } catch (error) {
