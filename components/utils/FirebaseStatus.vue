  ...
  <div class="p-4 bg-base-200 rounded-lg">
    ...
    <div class="space-y-2 mb-4">
      <div class="flex items-center gap-2">
        <div class="w-3 h-3 rounded-full" :class="firebaseReady ? 'bg-success' : 'bg-error'"></div>
        <span>Firebase: {{ firebaseReady ? 'Prêt' : 'Non initialisé' }}</span>
      </div>
      
      <div class="flex items-center gap-2">
        <div class="w-3 h-3 rounded-full" :class="authReady ? 'bg-success' : 'bg-warning'"></div>
        <span>Authentification: {{ authReady ? 'Disponible' : 'Non disponible' }}</span>
      </div>
      
      <div class="flex items-center gap-2">
        <div class="w-3 h-3 rounded-full" :class="analyticsReady ? 'bg-success' : 'bg-warning'"></div>
    ...
    <!-- Résultat du test -->
    <div v-if="testResult" class="mt-4 p-3 rounded" :class="testResult.success ? 'bg-success/20' : 'bg-error/20'">
      <div class="font-medium">{{ testResult.success ? '✅ Succès' : '❌ Erreur' }}</div>
      <div class="text-sm">{{ testResult.message }}</div>
    </div>
  </div>
</template>
