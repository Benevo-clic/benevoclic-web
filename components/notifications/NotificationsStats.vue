<template>
  <div class="bg-base-100 rounded-xl shadow-lg border border-base-300 p-6">
    <div class="flex items-center justify-between mb-6">
      <h3 class="text-lg font-semibold text-base-content">{{ $t('notifications.statistics') }}</h3>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <div class="stat bg-base-200 rounded-lg p-4 text-center hover:shadow-md transition-all duration-200">
        <div class="stat-value text-primary text-2xl font-bold">{{ stats.total }}</div>
        <div class="stat-desc text-sm text-base-content/70">{{ $t('notifications.total_notifications') }}</div>
      ...
      <div class="stat bg-base-200 rounded-lg p-4 text-center hover:shadow-md transition-all duration-200">
        <div class="stat-value text-warning text-2xl font-bold">{{ stats.unread }}</div>
        <div class="stat-desc text-sm text-base-content/70">{{ $t('notifications.unread_notifications') }}</div>
      ...
      <div class="stat bg-base-200 rounded-lg p-4 text-center hover:shadow-md transition-all duration-200">
        <div class="stat-value text-success text-2xl font-bold">{{ stats.today }}</div>
        <div class="stat-desc text-sm text-base-content/70">{{ $t('notifications.today') }}</div>
        <div class="stat-desc text-xs text-success mt-1">
    ...
    <div class="mb-6">
      <h4 class="text-md font-medium text-base-content mb-3">{{ $t('notifications.stats.by_type') }}</h4>
      <div class="space-y-2">
        <div 
        ...
        >
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full flex items-center justify-center" :class="type.bgColor">
              <component :is="type.icon" class="w-4 h-4" :class="type.iconColor" />
            </div>
            <div>
              <div class="font-medium text-base-content">{{ type.name }}</div>
              <div class="text-xs text-base-content/70">{{ type.description }}</div>
            </div>
          </div>
          <div class="text-right">
            <div class="font-semibold text-base-content">{{ type.count }}</div>
    ...
    <!-- Activité récente -->
    <div>
      <h4 class="text-md font-medium text-base-content mb-3">{{ $t('notifications.stats.recent_activity') }}</h4>
      <div class="space-y-2">
        <div 
        ...
        >
          ...
          <div class="flex-1">
            <div class="text-sm text-base-content">{{ activity.description }}</div>
            <div class="text-xs text-base-content/50">{{ formatTime(activity.time) }}</div>
          </div>
        </div>
      </div>
    </div>
...
const typeStats = computed(() => {
  ...
  return types.map(type => {
    ...
    const typeConfig = {
      message: { name: 'Messages', icon: MessageSquare, bgColor: 'bg-info/20', iconColor: 'text-info' },
      event: { name: 'Événements', icon: Calendar, bgColor: 'bg-primary/20', iconColor: 'text-primary' },
      mission: { name: 'Missions', icon: MapPin, bgColor: 'bg-accent/20', iconColor: 'text-accent' },
      achievement: { name: 'Réalisations', icon: Award, bgColor: 'bg-success/20', iconColor: 'text-success' },
      alert: { name: 'Alertes', icon: AlertCircle, bgColor: 'bg-warning/20', iconColor: 'text-warning' }
    }[type] || { name: 'Autres', icon: Bell, bgColor: 'bg-base-300', iconColor: 'text-base-content' }
