<template>
  <div class="bg-base-100 rounded-lg shadow-md p-4">
    ...
    <ul class="menu menu-vertical w-full">
      <li>
        <NuxtLink to="/volunteer/account/profile" class="flex items-center gap-2 p-2 rounded hover:bg-base-200">
          <UserRound class="w-5 h-5" />
          {{ t('drawer-content.account.view_profile') }}
        </NuxtLink>
      </li>
      <li>
        <NuxtLink to="/volunteer/account/edit" class="flex items-center gap-2 p-2 rounded hover:bg-base-200">
          <Pencil class="w-5 h-5" />
          {{ t('drawer-content.account.edit_profile') }}
        </NuxtLink>
      </li>
      <li>
        <NuxtLink to="/volunteer/account/settings" class="flex items-center gap-2 p-2 rounded hover:bg-base-200">
