<template>
  <div id="cookie-consent" v-if="showBanner" class="fixed bottom-0 left-0 right-0 bg-base-200 shadow-lg z-50 transition-all duration-300" :class="{ 'translate-y-0': showBanner, 'translate-y-full': !showBanner }">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div class="flex-1">
          ...
          <p class="text-sm text-base-content/80 mb-4 md:mb-0">
            Nous utilisons des cookies pour améliorer votre expérience sur notre site. Certains sont nécessaires au fonctionnement du site, 
            tandis que d'autres nous aident à comprendre comment vous l'utilisez. Vous pouvez accepter tous les cookies ou personnaliser vos préférences.
            <NuxtLink to="/confidentialite" class="text-primary hover:underline">En savoir plus</NuxtLink>
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          <button @click="openSettings" class="btn btn-outline btn-sm">Personnaliser</button>
          <button @click="acceptAll" class="btn btn-primary btn-sm">Accepter tout</button>
          <button @click="rejectNonEssential" class="btn btn-ghost btn-sm">Refuser non essentiels</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <dialog ref="settingsModal" class="modal">
    <div class="modal-box max-w-2xl">
          ...
          <label class="label cursor-pointer justify-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-base-content/70 mt-1">
                ...
                Ils permettent notamment de vous authentifier et de sécuriser votre navigation.
                <span class="text-warning font-medium">Sans ces cookies, vous ne pourrez pas vous connecter à votre compte.</span>
              </p>
            </div>
          </label>
        </div>
        ...
        <div class="form-control">
          <label class="label cursor-pointer justify-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-base-content/70 mt-1">
                Ces cookies nous permettent de vous proposer des contenus adaptés à vos centres d'intérêt,
                de mémoriser vos préférences d'affichage et d'utiliser votre localisation pour des services géolocalisés.
                <span class="text-info font-medium">Nécessaire pour la géolocalisation et les données personnelles.</span>
              </p>
            </div>
          </label>
        </div>
        ...
        <div class="form-control">
          <label class="label cursor-pointer justify-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-base-content/70 mt-1">
                Ces cookies nous permettent de mesurer l'audience de notre site et d'améliorer son fonctionnement.
                Ils sont utilisés par Firebase Analytics pour générer des statistiques anonymes.
              </p>
            </div>
          </label>
        </div>
        ...
        <div class="form-control">
          <label class="label cursor-pointer justify-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-base-content/70 mt-1">
                Ces cookies sont déposés par des services tiers comme Google Fonts ou les réseaux sociaux.
                Ils permettent notamment d'afficher des polices personnalisées et de partager du contenu.
              </p>
            </div>
          </label>
        </div>
      ...
      <div class="modal-action">
        <button @click="closeSettings" class="btn btn-ghost">Annuler</button>
      ...
      <div class="space-y-4">
        ...
        <div v-if="!permissions.canUseLocation" class="alert alert-info">
          ...
          <div>
            ...
            <p class="text-sm">Acceptez les cookies de personnalisation pour utiliser la géolocalisation.</p>
          </div>
        </div>
      </div>
      <div class="modal-action">
        <button @click="closePermissionModal" class="btn btn-ghost">Fermer</button>
        <button @click="openSettings" class="btn btn-primary">Paramétrer les cookies</button>
      </div>
    </div>
    <form method="dialog" class="modal-backdrop">
      <button>Fermer</button>
...
onMounted(async () => {
  try {
    ...
    watch(() => permissionsComposable.permissions.value, (newPermissions) => {
      permissions.value = newPermissions;
      
      // Si l'utilisateur a consenti mais n'a pas les permissions nécessaires
      if (permissionsComposable.hasConsented.value && (!newPermissions.canAuthenticate || !newPermissions.canUseLocation)) {
