...
const errors = reactive<
  Record<keyof(FormFieldsVolunteer & { nameFirstName: string }), string>
    >({
      lastName: '',
      firstName: '',
      nameFirstName: '',
...
function validateCurrentStep (): boolean {
  ...
  const error = step.validate(
    formData[
      step.field as keyof (FormFieldsVolunteer & { nameFirstName: string })
    ]
  )
  errors[
    step.field as keyof (FormFieldsVolunteer & { nameFirstName: string })
...
<template>
  <div class="w-full max-w-md mx-auto p-4">
    <h1 class="text-3xl font-bold mb-2">
      🚀 On démarre l'aventure
    </h1>
    <p class="text-base text-gray-600 mb-4">
      Dis-nous un peu sur toi étape par étape 😊
    </p>
