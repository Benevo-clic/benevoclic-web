...
function handleValidate() {
  ...
  const tagsArray = localFilters.value.tags
    ? (typeof localFilters.value.tags === 'string' ? localFilters.value.tags.split(',').map(t => t.trim()).filter(Boolean) : localFilters.value.tags)
    : [];
...
<template>
  <transition name="fade">
    <div v-if="open" class="fixed inset-0 bg-black bg-opacity-40 z-40" @click="handleClose" />
  </transition>
  <transition name="slide">
    <aside v-if="open" class="fixed top-0 right-0 h-screen w-full max-w-md bg-base-100 shadow-lg flex flex-col z-50 text-base-content">
      <div class="flex items-center justify-between pt-2 pl-2 pr-2 pb-2">
        <span class="font-bold text-lg">Filtres avancés</span>
        <button @click="handleClose" class="btn btn-ghost btn-square">
          <X class="w-6 h-6" />
        </button>
      </div>
