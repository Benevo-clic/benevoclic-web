...
const selectAddress = (address: AddressFeature) => {
  ...
  if (match) {
    ...
    const city     = match[3];
    query.value = street;
    formState.addressAnnouncement.address = street;
    formState.addressAnnouncement.postalCode = zipCode;
    formState.addressAnnouncement.city = city;
    formState.addressAnnouncement.country = 'France';
    formState.locationAnnouncement.coordinates = [lng, lat];

  } else {
    console.error("Le format de l'adresse n'est pas reconnu.");
  }
