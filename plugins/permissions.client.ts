  ...
  if (process.client) {
    ...
    localStorage.setItem = function(key: string, value: string) {
      ...
      if (personalDataKeys.some(k => key.includes(k)) && !hasPermission('canUsePersonalData')) {
        console.warn('Tentative d\'accès aux données personnelles bloquée - cookies de personnalisation requis')
        return
      }
    ...
    localStorage.getItem = function(key: string) {
      ...
      const personalDataKeys = ['user_location', 'user_preferences', 'user_profile']
      if (personalDataKeys.some(k => key.includes(k)) && !hasPermission('canUsePersonalData')) {
        console.warn('Tentative de lecture des données personnelles bloquée - cookies de personnalisation requis')
        return null
      }
